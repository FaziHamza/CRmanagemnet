...
export class AuthService {
  ...
  public loginUser(model: LoginModel) {
    ...
    let url = environment.Url + ApiConstants.apiEndPoints.auth.login;
  ...
  public userRegister(model: LoginModel) {
    ...
    let url = environment.Url + ApiConstants.apiEndPoints.auth.register;
    return this.http.post(url, model);
  }
  public confirmEmail(model: any) {
    // model["userName"] = model["email"];
    let url = environment.Url + ApiConstants.apiEndPoints.auth.confirmEmail;
    return this.http.post(url, model);
  }
  sendEmail(emailSendDto): Observable<AppResponse> {
    let url = environment.Url + "XEmail/SendEamil";
    return this.http.post<AppResponse>(url, emailSendDto);
  }
  GetUserList(): Observable<AppResponse> {
    let url = environment.Url + "Account/GetUserList";
    return this.http.get<AppResponse>(url);
  }
  public sendCodeAgain(model: any) {
    // model["userName"] = model["email"];
    let url = environment.Url + ApiConstants.apiEndPoints.auth.sendCodeAgain;
    return this.http.post(url, model);
  }
  public sendSMSAgain(model: any) {
    
    // model["userName"] = model["email"];
    let url = environment.Url + ApiConstants.apiEndPoints.auth.sendSMSAgain;
    return this.http.post(url, model);
  }
  public resetPassword(model: any) {
    let url = environment.Url + ApiConstants.apiEndPoints.auth.resetPassword;
    return this.http.post(url, model);
  }
}
